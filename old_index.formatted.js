...
var e = window.location.hostname === `localhost` || window.location.hostname === `127.0.0.1` ? `http://195.35.56.82:5024/api` : `https://api.askidagmtid.com/api`,
    t = {
...
async function p(n, r) {
    ...
    try {
        ...
        let s = a.data.user;
        if (!s || s.role !== `Admin`) throw Error(`Yetkisiz erişim. Sadece Yöneticiler giriş yapabilir.`);
        localStorage.setItem(`admin_session`, `true`), localStorage.setItem(`admin_user`, JSON.stringify(s)), localStorage.setItem(`admin_token`, a.data.token), t.isAuthenticated = !0, t.adminUser = s, o(`Giriş başarılı! Yönetim paneline yönlendiriliyorsunuz.`, `success`), f()
    } catch (e) {
        o(e.message, `error`), i && (i.disabled = !1, i.innerHTML = `Yönetici Girişi <i class="fas fa-arrow-right"></i>`)
    }
}

function m() {
    localStorage.removeItem(`admin_session`), localStorage.removeItem(`admin_user`), t.isAuthenticated = !1, t.adminUser = null, t.usersList = [], o(`Oturum kapatıldı.`, `info`), y()
}
async function h(n) {
    t.isActionLoading = !0, y();
    try {
        let r = await fetch(`${e}/users/${n}/approve`, {
                method: `POST`
            }),
            i = await r.json();
        if (!r.ok || !i.success) throw Error(i.message || `Kullanıcı onaylanamadı.`);
        o(`Kullanıcı başarıyla onaylandı!`, `success`), t.selectedUser = null, c()
    } catch (e) {
        o(e.message, `error`)
    } finally {
        t.isActionLoading = !1, y()
    }
}
async function g(n) {
    t.isActionLoading = !0, y();
    try {
        let r = await fetch(`${e}/users/${n}/reject`, {
                method: `POST`
            }),
            i = await r.json();
        if (!r.ok || !i.success) throw Error(i.message || `İşlem gerçekleştirilemedi.`);
        o(`Öğrenci belgesi reddedildi.`, `info`), t.selectedUser = null, c()
    } catch (e) {
        o(e.message, `error`)
    } finally {
        t.isActionLoading = !1, y()
...
async function _(n) {
    t.isActionLoading = !0, y();
    try {
        let r = await fetch(`${e}/users/${n}/unverify`, {
                method: `POST`
            }),
            i = await r.json();
        if (!r.ok || !i.success) throw Error(i.message || `Durum güncellenemedi.`);
        o(`Kullanıcı doğrulama durumu sıfırlandı (Onaysız yapıldı).`, `info`), t.selectedUser = null, c()
    } catch (e) {
        o(e.message, `error`)
    } finally {
        t.isActionLoading = !1, y()
...
async function v(n) {
    if (confirm(`Bu kullanıcıyı tamamen silmek istediğinize emin misiniz? Bu işlem geri alınamaz!`)) {
        t.isActionLoading = !0, y();
        try {
            let r = await fetch(`${e}/users/${n}`, {
                    method: `DELETE`
                }),
                i = await r.json();
            if (!r.ok || !i.success) throw Error(i.message || `Kullanıcı silinemedi.`);
            o(`Kullanıcı başarıyla silindi.`, `success`), t.selectedUser = null, c()
        } catch (e) {
            o(e.message, `error`)
        } finally {
            t.isActionLoading = !1, y()
...
function y() {
  ...
  `, T();
    let a = document.getElementById(`btn-refresh-data`);
...
function T() {
    ...
    d && t.selectedUser && d.addEventListener(`click`, () => {
        _(t.selectedUser.id)
    });
    let p = document.getElementById(`action-delete-modal-btn`);
    p && t.selectedUser && p.addEventListener(`click`, () => {
        v(t.selectedUser.id)
    });
    let b = document.getElementById(`btn-open-doc`);
    b && t.selectedUser && b.addEventListener(`click`, async () => {
        let n = t.selectedUser.verificationDocumentUrl || ``,
            r = n.toLowerCase();
        if (r.includes(`.heic`) || r.includes(`.heif`)) {
            o(`HEIF görseli dönüştürülüyor ve açılıyor...`, `info`);
            try {
                let e = await fetch(n);
                if (!e.ok) throw Error(`File download failed`);
                let t = await e.blob();
                if (typeof heic2any < `u`) {
                    let e = await heic2any({
                            blob: t,
                            toType: `image/jpeg`,
                            quality: .8
                        }),
                ...
                } else throw Error(`heic2any library not loaded`)
            } catch (e) {
                console.error(`HEIF open error, falling back:`, e), window.open(n, `_blank`)
            }
        } else {
            let n = `${e}/users/${t.selectedUser.id}/view-document`;
            window.open(n, `_blank`)
        }
    });
    let x = document.getElementById(`btn-download-doc`);
    x && t.selectedUser && x.addEventListener(`click`, () => {
        let n = `${e}/users/${t.selectedUser.id}/download-document`;
        o(`Dosya indiriliyor...`, `info`);
        let r = document.createElement(`a`);
        r.href = n, document.body.appendChild(r), r.click(), document.body.removeChild(r)
    }), document.querySelectorAll(`.btn-match-deliver`).forEach(t => {
        t.addEventListener(`click`, async () => {
            let n = t.getAttribute(`data-id`),
                r = document.querySelector(`.select-donation-${n}`),
                i = r ? r.value : null;
            if (!i) {
                o(`Lütfen teslimat için bir bağış seçin.`, `error`);
                return
            }
            t.disabled = !0, t.innerHTML = `<i class="fas fa-spinner fa-spin"></i> İşleniyor...`;
            try {
                let t = await fetch(`${e}/requests/${n}/deliver`, {
                        method: `POST`,
                        headers: {
                            "Content-Type": `application/json`
                        },
                    ...
                    r = await t.json();
                if (!t.ok || !r.success) throw Error(r.message || `Teslimat onaylanamadı.`);
                o(r.message, `success`), f()
            } catch (e) {
                o(e.message, `error`), t.disabled = !1, t.innerHTML = `<i class="fas fa-check-circle"></i> Eşleştir ve Teslim Et`
            }
        })
    }), document.querySelectorAll(`.btn-approve-request`).forEach(t => {
        t.addEventListener(`click`, async () => {
            let n = t.getAttribute(`data-id`);
            try {
                let t = await fetch(`${e}/requests/${n}/approve`, {
                        method: `POST`
                    }),
                    r = await t.json();
                if (!t.ok || !r.success) throw Error(r.message || `Onaylanamadı.`);
                o(r.message, `success`), f()
            } catch (e) {
                o(e.message, `error`)
            }
        })
    }), document.querySelectorAll(`.btn-reject-request`).forEach(t => {
        t.addEventListener(`click`, async () => {
            let n = t.getAttribute(`data-id`);
            try {
                let t = await fetch(`${e}/requests/${n}/reject`, {
                        method: `POST`
                    }),
                    r = await t.json();
                if (!t.ok || !r.success) throw Error(r.message || `Reddedilemedi.`);
                o(r.message, `success`), f()
            } catch (e) {
                o(e.message, `error`)
            }
        })
    }), document.querySelectorAll(`.btn-delete-product`).forEach(t => {
        t.addEventListener(`click`, async () => {
            if (!confirm(`Bu ürünü silmek istediğinize emin misiniz?`)) return;
            let n = t.getAttribute(`data-id`);
            try {
                if (!(await fetch(`${e}/products/${n}`, {
                        method: `DELETE`
                    })).ok) throw Error(`Ürün silinemedi.`);
                o(`Ürün silindi.`, `success`), f()
            } catch (e) {
                o(e.message, `error`)
            }
        })
    });
    let S = document.getElementById(`btn-add-product`);
    S && S.addEventListener(`click`, async () => {
        let t = prompt(`Ürün Adı:`);
        if (!t) return;
        let n = prompt(`Fiyat (TL):`);
        if (!n) return;
        let r = parseFloat(n);
        if (isNaN(r)) {
            o(`Geçersiz fiyat`, `error`);
            return
        }
        let i = prompt(`Açıklama:`);
        try {
            if (!(await fetch(`${e}/products`, {
                    method: `POST`,
                    headers: {
                        "Content-Type": `application/json`
                    },
                    body: JSON.stringify({
                        ...
                        imageUrl: `https://images.unsplash.com/photo-1550547660-d9450f859349?q=80&w=200&auto=format&fit=crop`
                    })
                })).ok) throw Error(`Ürün eklenemedi.`);
            o(`Ürün başarıyla eklendi.`, `success`), f()
        } catch (e) {
            o(e.message, `error`)
        }
    })
}
t.isAuthenticated ? f() : y();